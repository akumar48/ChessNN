...
class MLP:
    ...
    def forward_propagation(self, inputs):
        hidden_outputs = torch.tensor(inputs, requires_grad=False).to(device)  # Move inputs to GPU
        for neuron in self.input_layer:
            hidden_outputs = torch.tensor([neuron.activate(hidden_outputs) for neuron in self.input_layer]).to(device)  # Move hidden_outputs to GPU
        for hidden_layer in self.hidden_layers:
            hidden_outputs = torch.tensor([neuron.activate(hidden_outputs) for neuron in hidden_layer]).to(device)  # Move hidden_outputs to GPU
        final_outputs = torch.tensor([neuron.activate(hidden_outputs) for neuron in self.output_layer]).to(device)  # Move final_outputs to GPU
        return final_outputs

    def train(self, inputs, targets, learning_rate=0.1):
        inputs = inputs.clone().detach().to(device)  # Move inputs to GPU
        targets = targets.clone().detach().to(device)  # Move targets to GPU
...
if __name__ == "__main__":
    ...
    # Move each individual neuron to the GPU
    for neuron in mlp.input_layer + [neuron for hidden_layer in mlp.hidden_layers for neuron in hidden_layer] + mlp.output_layer:
        neuron.weights = neuron.weights.to(device)
        neuron.bias = neuron.bias.to(device)
    ...
    for epoch in range(epochs):
        ...
        for inputs, targets in training_data:
            inputs = torch.tensor(inputs).to(device)  # Move inputs to GPU
            targets = torch.tensor(targets).to(device)  # Move targets to GPU
            mlp.train(inputs, targets)
        elapsed_time = time.time() - start_time
        print(f"Time elapsed for epoch {epoch + 1}: {elapsed_time:.2f} seconds")

    model_filename = f"nn2tg_{training_data_filename}_{test_data_filename}.txt"
